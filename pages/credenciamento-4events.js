...
const ConfigurationScreen = ({ onSessionStart }) => {
  ...
  useEffect(() => {
    ...
    fetch('/api/events', {
      ...
      .catch(() => setError('Erro de rede ao buscar eventos.'))
  ...
  useEffect(() => {
    if (selectedEvent) {
      ...
      setTicketCategories([]);
      setSelectedTicket('');
      fetch(`/api/ticket-categories?eventId=${selectedEvent}`)
        .then((res) => res.json())
        .then((data) => {
          if (Array.isArray(data)) {
        ...
        .catch(() => setError('Erro de rede ao buscar ingressos.'))
        .finally(() => setLoading(false));
    } else {
      setTicketCategories([]);
    }
    ...
    <div className="app-container">
      <div className="card">
        ...
        <div className="form-group">
          ...
          <button onClick={handleStart} disabled={isDisabled} className="btn btn-primary">
...
const ConfirmationScreen = ({ initialParticipant, onCancel, session, onCredentialingSuccess }) => {
  ...
  useEffect(() => {
    ...
    if (
      ...
      initialParticipant.rawData?.ListaVinculo &&
    ...
    <form onSubmit={handleCredentialing} className="space-y-4 animate-fade-in">
      ...
      <div>
        ...
        <input
          ...
          onChange={handleInputChange}
          className={`w-full px-4 py-3 mt-1 border rounded-xl shadow-sm ${emailHint ? 'border-red-500' : 'border-gray-300'}`}
          maxLength={254}
        />
      </div>
      <div>
      ...
      <button type="submit" disabled={loading || !consentGiven} className="btn btn-success">
        {loading ? 'Processando...' : 'Confirmar e Credenciar'}
      </button>
      <button type="button" onClick={onCancel} className="btn btn-secondary mt-2">
        Cancelar
      </button>
    </form>
  );
          ...
          <div className="bg-white/10 backdrop-blur-sm border border-white/20 rounded-xl p-8">
            ...
            {success ? (
              <SuccessScreen
                ...
                onCopyAndNewSearch={handleCopyAndOpenPanel}
                email={lastEmail}
              />
            ) : alreadyRegistered ? (
              <div className="text-center space-y-6 animate-fade-in">
                ...
                <div className="text-left bg-white/10 border border-white/20 backdrop-blur-sm p-4 rounded-xl">
                  <p className="text-white">
                    <strong>Nome:</strong> {alreadyRegistered.name}
                  </p>
                  <p className="text-white">
                    <strong>E-mail:</strong> {alreadyRegistered.email}
                  </p>
                </div>
                <button

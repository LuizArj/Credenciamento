    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col items-center space-y-6">
        ...
        <div className="w-full">
          <label className="block text-white font-medium mb-2">Nome na Etiqueta</label>
          <input
            type="text"
            value={displayName}
        ...
        <div className="w-full flex flex-col gap-2">
          ...
          <button onClick={onNewSearch} className="btn btn-secondary w-full">
            Nova Pesquisa
          </button>
        </div>
      </div>
      ...
      if (searchRes.status === 404) {
        ...
        if (searchData.ListaVinculo?.length > 0) {
          const vinculoPrincipal =
            searchData.ListaVinculo.find((v) => v.IndPrincipal === 1) || searchData.ListaVinculo[0];
      ...
      <main className="flex-1 flex items-center justify-center px-4 py-8">
        <div className="w-full max-w-2xl">
          <div className="bg-white/10 backdrop-blur-sm border border-white/20 rounded-xl p-8">
            ...
            {confirmedData ? (
              <QRCodeLabel participantData={confirmedData} onNewSearch={handleNewSearch} />
            ) : participant ? (
              <ConfirmationScreen

  ...
  try {
    ...
    if (empresaNome) {
      ...
      if (companyRes.rows.length > 0) {
        ...
        warnings.push(
          `Linha ${rowIndex}: Empresa "${empresaNome}" não encontrada, participante será criado sem empresa`
        );
      }
    }
    ...
    const email =
      String(row.EMAIL || row.email || '').trim() || `${cpf.replace(/\D/g, '')}@temp.com`;
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!userRoles.includes('admin')) {
      return res
        .status(403)
        .json({ error: 'Sem permissão. Apenas administradores podem importar dados.' });
    }

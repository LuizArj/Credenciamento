...
async function handler(req, res) {
  ...
  try {
    ...
    const eventsRes = await query(
      'SELECT id, nome, status, data_inicio, data_fim, capacidade, created_at FROM events WHERE ativo = true AND created_at >= $1',
    ...
    const partRes = await query(
      'SELECT COUNT(*)::int AS total FROM participants WHERE ativo = true'
    );
    ...
    if (eventIds.length > 0) {
      const regsRes = await query(
        `SELECT event_id, status, COUNT(*)::int AS count FROM registrations WHERE event_id = ANY($1) GROUP BY event_id, status`,
        [eventIds]
      );
      ...
      const checkRes = await query(
        `SELECT r.event_id, COUNT(ci.*)::int AS count FROM check_ins ci JOIN registrations r ON ci.registration_id = r.id WHERE r.event_id = ANY($1) GROUP BY r.event_id`,
        [eventIds]
      );
    ...
    const attendanceRate =
      totalRegistrations > 0 ? Math.round((checkedInRegistrations / totalRegistrations) * 100) : 0;

    // Buscar check-ins recentes com mais detalhes
    ...
    const recentCheckIns = (recentRes.rows || []).map((ci) => ({
      ...
      participantCpf: ci.participant_cpf,
      eventName: ci.event_name,
      eventLocation: ci.event_location || 'Local não informado',
      checkInTime: ci.data_check_in,
      time: ci.data_check_in, // Mantém compatibilidade
    ...
    const eventStats = [];
    if (eventIds.length > 0) {
      const regCountsRes = await query(
        `SELECT event_id, COUNT(*)::int AS registrations_count FROM registrations WHERE event_id = ANY($1) GROUP BY event_id`,
        [eventIds]
      );
      const regCounts = regCountsRes.rows || [];
      const regMap = Object.fromEntries(regCounts.map((r) => [r.event_id, r.registrations_count]));

      const checkedCountsRes = await query(
        `SELECT r.event_id, COUNT(ci.*)::int AS checked_in FROM check_ins ci JOIN registrations r ON ci.registration_id = r.id WHERE r.event_id = ANY($1) GROUP BY r.event_id`,
        [eventIds]
      );
      const checkedMap = Object.fromEntries(
        (checkedCountsRes.rows || []).map((r) => [r.event_id, r.checked_in])
      );
    ...
    const chartRes = await query(
      `SELECT (date_trunc('day', data_check_in))::date AS day, COUNT(*)::int AS count FROM check_ins WHERE data_check_in >= $1 GROUP BY day ORDER BY day`,
      [sinceDate.toISOString()]
    );
    const countsByDay = Object.fromEntries(
      (chartRes.rows || []).map((r) => [r.day.toISOString().split('T')[0], r.count])
    );
    const labels = [
      '6 dias atrás',
      '5 dias atrás',
    ...
    const topRes = await query(`
      ...
      LIMIT 5
    `);
    const topCompanies = (topRes.rows || []).map((company) => ({
      name: company.nome_fantasia || company.razao_social,
      participants: company.participants || 0,
    }));
    ...
    return res.status(200).json({
      ...
      recentCheckIns: [],
      eventStats: [],
      chartData: [{ date: new Date().toISOString().split('T')[0], count: 0, label: 'Hoje' }],
      topCompanies: [],
      period: 'month',
      generatedAt: new Date().toISOString(),
    });
